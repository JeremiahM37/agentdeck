...
transport.  The shell is tracked by AgentDeck, but it has no project or agent
...
def _pty_cli(t, *args):
    return _spawn_process(_binary(), {**t["env"], "AGENTDECK_API": t["url"]}, *args)
...
def test_cli_shell_picks_local_machine_and_keeps_blank_room(real_terminal):
    ...
    try:
        ...
        scratch_root = Path(t["env"]["HOME"]) / "agentdeck-scratch"
...
def test_cli_shell_runs_on_the_selected_ssh_machine(remote_terminal):
    ...
    try:
        ...
        assert row.get("project_id") is None
        assert row["workdir"].startswith(str(t["remote_home"] / "agentdeck-scratch"))

        os.write(master, b"printf ssh-proof > quick-shell-ssh.txt\r")
        # The scratch suffix is generated by AgentDeck.  Resolve the exact
...
def test_tui_quick_shell_action_is_real_and_cancellable(real_terminal):
    ...
    output = b""
    try:
        output = _read_until(master, b"AgentDeck", 20, output)
        os.write(master, b"?")
        ...
        os.write(master, b"\x02d")
        output = _read_until(master, b"AgentDeck", 20, b"")
...
def test_installed_local_shell_uses_private_runtime(local_binary, tmp_path):
    """The installed ``agentdeck local shell`` path works without a server."""
    ...
    try:
        ...
        assert row["workdir"].startswith(str(root / "home" / "agentdeck-scratch"))
