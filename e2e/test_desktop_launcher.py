...
SCRIPT=ROOT/'web/static/desktop/setup-agentdeck-terminal.sh'
...
def test_rejects_invalid_links_before_launch(tmp_path,uri):
    ...
    assert p.returncode==2 and 'Invalid AgentDeck' in p.stderr

def test_legacy_download_also_uses_default_terminal():
    assert SCRIPT.read_bytes()==(SCRIPT.parent/'setup-agentdeck-kitty.sh').read_bytes()
