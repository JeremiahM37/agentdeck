...
func (db *DB) PromoteNewProjectAndBind(ctx context.Context, p *Project, sessionID int64, tmux, boot, tracking, cid, launchConfig string) (*Project, error) {
	if p.MemoryTopic == "" {
		b := make([]byte, 16)
	...
	result, err := tx.ExecContext(ctx, `UPDATE sessions SET project_id=?, workdir=?, agent=?, native_recovery_cid=?, launch_config_json=? WHERE id=? AND ended_at IS NULL AND (project_id IS NULL) AND tmux_session=? AND boot_id=? AND tracking_identity=?`, id, p.RepoPath, p.DefaultAgent, cid, launchConfig, sessionID, tmux, boot, tracking)
	if err != nil {
		return nil, err
	}
