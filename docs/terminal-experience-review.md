...
| Organize large fleets | Named group paths now persist across projects/targets and inherit on forks/handoffs. TUI supports editing/group ordering/search; web adds nested collapse/counts and persistent per-tab display state. Profile configuration, archive/recovery, TUI tree collapse and global conversation search remain. |
