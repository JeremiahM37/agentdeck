...
export function serviceWorkerPlugin():Plugin{return {name:'typed-service-worker',generateBundle(_options,bundle){
 ...
 this.emitFile({type:'asset',fileName:'sw.js',source:output.replaceAll('__CACHE_NAME__',JSON.stringify('agentdeck-react-'+version)).replaceAll('__STATIC_ASSETS__',JSON.stringify(['/','/icon.svg','/manifest.webmanifest','/fonts.css','/fonts/inter-latin.woff2','/fonts/inter-latin-ext.woff2',...assets.map(name=>'/react/'+name)]))});
}};}
